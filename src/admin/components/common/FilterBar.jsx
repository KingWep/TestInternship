...
function CustomSelect({ filter, value, onChange }) {
  ...
  return (
    <div className="relative w-full sm:w-auto min-w-[130px]" ref={ref}>
      <button
        type="button"
      ...
      >
        <span className="truncate">{value || `ជ្រើសរើស ${filter.key}`}</span>
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((option) => (
                <li
                  ...
                  className={`px-3 py-2 text-xs sm:text-sm cursor-pointer hover:bg-slate-50 ${value === option ? 'bg-blue-50 text-blue-600 font-medium' : 'text-slate-700'}`}
                >
                  {option}
                </li>
              ))
            ) : (
              <li className="px-3 py-2 text-xs text-slate-500 text-center">គ្មានលទ្ធផល</li>
            )}
          </ul>
        </div>
      )}
