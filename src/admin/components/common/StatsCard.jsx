...
export default function StatsCard({ title, value, icon: Icon, trend, color = 'blue', warning, note }) {
  const style = colorStyles[color] || colorStyles.blue

  return (
    <div className={`bg-white border border-slate-200 border-l-4 ${style.border} rounded-2xl p-5`}>
      ...
      {trend !== undefined && trend !== null ? (
        <p className={`text-xs font-semibold flex items-center gap-1 ${Number(trend) >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
          <span>{Number(trend) >= 0 ? '+' : ''}{trend}%</span>
          <span className="text-slate-400 font-normal">vs last month</span>
        </p>
      ) : note ? (
        <p className={`text-xs ${warning ? 'text-amber-600' : 'text-slate-400'}`}>{note}</p>
      ) : null}
    </div>
  )
}